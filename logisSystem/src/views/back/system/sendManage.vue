<template>
  <div>
    ...
    <el-dialog title="发出货物" v-model="transIsShow">
      <div style="width: 95%;position: center">订单签收成功!</div>
      <span class="dialog-footer">
        <el-button type="primary" @click="transIsShow = false">确认</el-button>
      </span>
    </el-dialog>
...
export default {
  ...
  methods: {
    async pickUp(row){
      ...
      await request.post('/system/ProcessRecords/signFor',axiosData).then(async res => {
    ...
    async getUserList () {
      ...
      await request.post('/system/ProcessRecords/signForList',axiosData).then(res=>{
        if(res.code===-20001||res.code==-20002){
          that.$router.push({name:'login'})
        }else{
